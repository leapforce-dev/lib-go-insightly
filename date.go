...
type DateBQ struct {
	DATE_ID            int        `json:"DATE_ID"`
	OCCASION_NAME      string     `json:"OCCASION_NAME"`
	OCCASION_DATE      *time.Time `json:"OCCASION_DATE"`
	REPEAT_YEARLY      bool       `json:"REPEAT_YEARLY"`
	CREATE_TASK_YEARLY bool       `json:"CREATE_TASK_YEARLY"`
}

func (d *Date) ToBQ() DateBQ {
	return DateBQ{
		d.DATE_ID,
		d.OCCASION_NAME,
		d.OccasionDate,
		d.REPEAT_YEARLY,
		d.CREATE_TASK_YEARLY,
	}
}
...
func (d *Date) ParseDates() {
	...
	if d.OCCASION_DATE != "" {
		t, err := time.Parse("2006-01-02 15:04:05 +0000 UTC", d.OCCASION_DATE+" +0000 UTC")
